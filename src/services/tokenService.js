...
class TokenService {
  ...
  async storeSession(accessToken, refreshToken) {
    try {
      ...
      await firestore().collection("userSessions").doc(`${user.uid}_${this.deviceId}`).set(sessionData, { merge: true })
    } catch (error) {
  ...
  async getStoredTokens() {
    try {
      const tokens = await AsyncStorage.multiGet(["access_token", "refresh_token", "token_timestamp"])
  ...
  async _performTokenRefresh() {
    try {
      ...
      const sessionDoc = await firestore().collection("userSessions").doc(`${user.uid}_${this.deviceId}`).get()
  ...
  async updateSessionActivity() {
    try {
      ...
      await firestore().collection("userSessions").doc(`${user.uid}_${this.deviceId}`).update({
        lastActivity: firestore.FieldValue.serverTimestamp(),
      })
    } catch (error) {
  ...
  async clearTokens() {
    try {
      await AsyncStorage.multiRemove(["access_token", "refresh_token", "token_timestamp"])
  ...
  async logoutDevice(userId, deviceId) {
    try {
      await firestore().collection("userSessions").doc(`${userId}_${deviceId}`).update({
        isActive: false,
        deactivatedAt: firestore.FieldValue.serverTimestamp(),
      })
    } catch (error) {
